...
export interface ITenantAuthService {
  registerTenant(
    data: ITenantRegisterRequest,
  ): Promise<ITenantRegisterResponse>;
  verifyTenantOtp(
    data: IVerifyTenantOtpRequest,
  ): Promise<IVerifyTenantOtpResponse>;
  resendTenantOtp(
    data: IResendTenantOtpRequest,
  ): Promise<IResendTenantOtpResponse>;
  updateBusinessInfo(
    data: IUpdateBusinessInfoRequest,
  ): Promise<IUpdateBusinessInfoResponse>;
...
class TenantAuthService implements ITenantAuthService {
  ...
  async loginTenant(data: ITenantLoginRequest): Promise<ITenantLoginResponse> {
    const response = await api.post<{ data: ITenantLoginResponse }>(
      API_ROUTES.TENANTS.LOGIN,
      data,
    );
    return response.data.data;
  }
  ...
  async getTenantProfile(): Promise<ITenantProfile> {
    const response = await api.get<{ data: { tenant: ITenantProfile } }>(
      API_ROUTES.TENANTS.GET_PROFILE,
    );
