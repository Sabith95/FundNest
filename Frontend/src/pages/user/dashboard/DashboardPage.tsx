  ...
  if (isWallet) {
    return (
      <div
        ...
        style={{
          background: "linear-gradient(135deg, #1a78d4 0%, #0ea5e9 100%)",
        }}
      >
  ...
  return (
    <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between mb-4">
        ...
        <span
          className={`text-[10px] font-bold tracking-wider uppercase px-2 py-1 rounded-lg ${badgeBg} ${badgeColor}`}
        >
          {badge}
        </span>
      </div>
    ...
    <div className="bg-white rounded-2xl border border-gray-100 p-4 hover:shadow-md transition-all">
      ...
      <div>
        <div className="flex items-center justify-between text-[10px] text-gray-400 font-bold tracking-wider uppercase mb-1.5">
          <span>
            Progress (Paid {fund.paidInstallments}/{fund.totalInstallments})
          </span>
          <span>{progress}%</span>
        </div>
...
const ActivityIcon: React.FC<{ type: string; status: string }> = ({
  ...
  return (
    <div
      className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${cfg.bg} ${cfg.color}`}
    >
      {cfg.icon}
    </div>
  );
...
const AlertCard: React.FC<{
  ...
  return (
    <div
      className={`bg-white rounded-xl p-4 border border-gray-100 border-l-4 ${cfg.border}`}
    >
      <div className="flex items-start gap-2.5">
        <div
          className={`w-5 h-5 rounded-full ${cfg.dot} flex items-center justify-center flex-shrink-0 mt-0.5`}
        >
          {alert.type === "urgent" ? (
...
const DashboardPage: React.FC<IDashboardPageProps> = ({
  ...
  const [data, setData] = useState<IDashboardData>(
    initialData ?? DEFAULT_DASHBOARD_DATA,
  );
              ...
              ? [
                  {
                    ...
                    title: "Complete KYC",
                    description:
                      "Upload your identity documents to unlock all FundNest features.",
        ...
        <div className="p-5 sm:p-6 lg:p-7">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400 mt-0.5">
                {isLoadingProfile
                  ...
                  : "Welcome to FundNest! Let's get you started."}
              </p>
            </div>
          ...
          <div className="flex flex-col items-center justify-center text-center py-16 px-6 bg-white rounded-2xl border border-gray-100 shadow-sm max-w-lg mx-auto">
            ...
            <p className="text-sm text-gray-400 leading-relaxed mb-8 max-w-xs">
              You haven't joined any chit fund yet. Browse available funds and
      ...
      <div className="p-5 sm:p-6 lg:p-7 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <StatCard
            ...
            badgeBg={
              data.stats.pendingAmount > 0 ? "bg-red-50" : "bg-orange-50"
            }
            badgeColor={
              data.stats.pendingAmount > 0 ? "text-red-600" : "text-orange-500"
            }
            label="Pending Payments"
            value={`₹${data.stats.pendingAmount.toLocaleString("en-IN")}`}
            subText={data.stats.pendingDueText || "Check payments"}
            subColor={
              data.stats.pendingAmount > 0 ? "text-red-500" : "text-gray-400"
            }
          />
          <StatCard
            icon={<WalletIcon />}
