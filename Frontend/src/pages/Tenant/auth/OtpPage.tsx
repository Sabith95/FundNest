...
const OtpBox: React.FC<IOtpBoxProps> = ({
  value,
  index,
  error,
  inputRef,
...
const OtpVerificationPage: React.FC = () => {
  ...
  const location = useLocation() as {
    state?: { email?: string; phone?: string; tenant?: ITenant };
  };
  ...
  const handleVerify = async () => {
    ...
    setError(undefined);
    try {
      const result = await tenantAuthService.verifyTenantOtp({
        email,
        otp: code,
      });
      ...
      >
        <div className="w-full max-w-[440px] px-5 py-10 sm:py-14">
          ...
          >
            ...
            {/* OTP boxes */}
            <div
              className="flex items-center justify-center gap-2 sm:gap-3 w-full mb-4"
              onPaste={handlePaste}
            >
              {digits.map((d, i) => (
                <OtpBox
            ...
            {error && (
              <p className="text-[12px] text-red-500 font-medium mb-2 -mt-1">
                {error}
              </p>
            )}
