...
export default function VerificationRejected({
  ...
  const busDocStatus =
    profile?.kycDocuments?.businessRegistrationCertificate?.verification
      ?.status;
  const busDocReason =
    profile?.kycDocuments?.businessRegistrationCertificate?.verification
      ?.rejectionReason;
  ...
  const ownerIdStatus =
    profile?.kycDocuments?.ownerIdProof?.verification?.status;
  const ownerIdReason =
    profile?.kycDocuments?.ownerIdProof?.verification?.rejectionReason;
  ...
  const isKycRejected =
    busDocStatus === "REJECTED" || ownerIdStatus === "REJECTED";
      ...
      <div className="w-full max-w-2xl rounded-2xl border border-red-200 bg-white p-6 shadow-sm sm:p-10">
        ...
        <div className="mt-6 text-center">
          <h1 className="text-xl font-bold tracking-tight text-slate-900 sm:text-2xl">
            {tenantName
              ? `Action Required, ${tenantName}`
              : "Action Required: Verification Rejected"}
          </h1>
          <p className="mx-auto mt-2 max-w-lg text-sm leading-relaxed text-slate-600">
            Our compliance team reviewed your submission and identified items
            that require correction before your account can be activated.
          </p>
        </div>
            ...
            <div className="flex flex-col justify-between rounded-xl border border-amber-200 bg-amber-50/50 p-4 sm:flex-row sm:items-center">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  {busDocReason && (
                    <p className="text-xs text-amber-800 mt-0.5">
                      Registration Cert: {busDocReason}
                    </p>
                  )}
                  {ownerIdReason && (
                    <p className="text-xs text-amber-800 mt-0.5">
                      Owner ID: {ownerIdReason}
                    </p>
                  )}
                </div>
              </div>
