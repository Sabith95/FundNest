...
const BankingDetails: React.FC = () => {
  ...
  const isFirstTimeSetup = !verificationStatus;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isResubmit) {
        toast.success("Bank details resubmitted for review.");
            ...
            >
              <div className="mb-8 flex flex-col items-center text-center">
                ...
                <h2 className="text-xl font-bold text-slate-900 sm:text-2xl">
                  {isResubmit ? "Update Banking Details" : "Bank Details"}
                </h2>
                <p className="mt-2 max-w-md text-sm text-slate-500 sm:text-base">
                  {isResubmit
                    ? "Correct the details below and resubmit for review."
                    : "Please provide your institutional bank details to facilitate secure fund distributions and settlements."}
                </p>
              </div>

              {/* Rejection banner — only for actually rejected verifications */}
