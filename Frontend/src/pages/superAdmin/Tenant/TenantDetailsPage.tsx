...
import { Landmark, ChevronRight, ShieldCheck, CheckCircle2, AlertCircle, Loader2, CreditCard } from "lucide-react";
import Header from "../../../components/admin/Header";
import Sidebar from "../../../components/admin/Sidebar";
import { adminTenantService } from "../../../services/adminTenantService";
import type { TenantDetailsData, VerificationState } from "../../../types/tenant.types";
      ...
      <div className="flex">
        ...
        <div className="flex min-h-screen w-full flex-1 flex-col lg:w-[calc(100%-16rem)]">
          ...
          <main className="flex-1 px-4 py-6 sm:px-6 lg:px-8">
            {/* Breadcrumb */}
            <nav className="mb-4 flex items-center gap-1.5 text-xs text-slate-400 sm:text-sm" aria-label="Breadcrumb">
              <span>Dashboard</span>
            ...
            {loading ? (
              <div className="flex h-64 flex-col items-center justify-center gap-3">
                ...
                <p className="text-sm font-medium text-slate-500">Loading tenant details...</p>
              </div>
            ) : error ? (
              ...
              <>
                {/* Page heading */}
                <div className="mb-6">
                  <h1 className="text-2xl font-bold text-slate-900 sm:text-3xl">Tenant Details</h1>
                  <p className="mt-1 text-sm text-slate-500">
                    Manage control settings and view verification history for {tenant.name}.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 gap-5 xl:grid-cols-3">
                  ...
                  <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm xl:col-span-2 sm:p-6">
                    <div className="flex flex-wrap items-start justify-between gap-3 border-b border-slate-100 pb-5">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h2 className="text-lg font-bold text-slate-900">{tenant.name || "N/A"}</h2>
                          <p className="text-sm text-slate-400">Tenant ID: {tenant.id}</p>
                        </div>
                      </div>
                      <span
                        className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold ${
                          statusBadgeClasses[tenant.status?.toUpperCase()] || "bg-slate-100 text-slate-700"
                        }`}
                      >
                    ...
                    <dl className="grid grid-cols-1 gap-x-6 gap-y-5 pt-5 sm:grid-cols-2">
                      <div>
                        <dt className="text-xs font-medium uppercase tracking-wide text-slate-400">Primary Owner</dt>
                        <dd className="mt-1 text-sm font-semibold text-slate-800">{tenant.primaryOwner || "N/A"}</dd>
                      </div>
                      <div>
                        <dt className="text-xs font-medium uppercase tracking-wide text-slate-400">Email Address</dt>
                        <dd className="mt-1 truncate text-sm font-semibold text-slate-800">{tenant.email || "N/A"}</dd>
                      </div>
                      <div>
                        <dt className="text-xs font-medium uppercase tracking-wide text-slate-400">Phone Number</dt>
                        <dd className="mt-1 text-sm font-semibold text-slate-800">{tenant.phone || "N/A"}</dd>
                      </div>
                      <div>
                        <dt className="text-xs font-medium uppercase tracking-wide text-slate-400">Registration Date</dt>
                        <dd className="mt-1 text-sm font-semibold text-slate-800">{tenant.registrationDate || "N/A"}</dd>
                      </div>
                    </dl>
                  </section>
                  ...
                  <section className="rounded-2xl border border-slate-200 bg-slate-50/60 p-5 shadow-sm sm:p-6">
                    <div className="mb-4 flex items-center gap-2">
                      ...
                      <h3 className="text-base font-bold text-slate-900">Verification Status</h3>
                    </div>

                    <ul className="space-y-2.5">
                      {(tenant.verification || []).map((item) => {
                        const itemStateKey = (item.state || "PENDING").toUpperCase() as VerificationState;
                        return (
                          <li
                            key={item.label}
                          ...
                          >
                            ...
                            <span
                              className={`rounded-md px-2 py-0.5 text-[11px] font-bold tracking-wide ${
                                verificationBadgeClasses[itemStateKey] || "bg-slate-100 text-slate-700"
                              }`}
                            >
                              {item.state}
                            </span>
                    ...
                    <button
                      type="button"
                      onClick={() => navigate(`/superadmin/tenants/${tenantId}/kyc`)}
                ...
                <div className="mt-5 grid grid-cols-1 gap-5 lg:grid-cols-2">
                  ...
                  <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
                    ...
                    <p className="mt-0.5 text-sm text-slate-400">Current billing cycle details</p>

                    {tenant.subscription && tenant.subscription.isActive ? (
                      <div className="mt-4 rounded-xl border border-indigo-100 bg-indigo-50 p-4">
                        <p className="text-sm font-semibold text-indigo-700">{tenant.subscription.planName}</p>
                        ...
                        <div className="mt-2 flex items-center gap-1.5 text-xs font-medium text-slate-500">
                          ...
                          Active • Next renewal {tenant.subscription.nextRenewalDate}
                        </div>
                      </div>
                    ) : (
                      <div className="mt-4 flex flex-col items-center justify-center rounded-xl border border-slate-200 bg-slate-50 p-6 text-center">
                        ...
                        <p className="mt-1 text-xs text-slate-500">
                          This tenant does not currently have an active subscription plan.
                        </p>
                      </div>
                    )}
                    ...
                    {tenant.financials ? (
                      <>
                        <div className="mt-4 flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-[11px] font-medium uppercase tracking-wide text-slate-400">Bank Name</p>
                            <p className="text-sm font-bold text-slate-900">{tenant.financials.bankName}</p>
                          </div>
                        </div>

                        <div className="mt-4 space-y-3 border-t border-slate-100 pt-4">
                          <div>
                            <p className="text-[11px] font-medium uppercase tracking-wide text-slate-400">Account Holder</p>
                            <p className="mt-0.5 text-sm font-semibold text-slate-800">{tenant.financials.accountHolder}</p>
                          </div>
                          <div>
                            <p className="text-[11px] font-medium uppercase tracking-wide text-slate-400">Account Number</p>
                      ...
                      <div className="mt-4 flex flex-col items-center justify-center rounded-xl border border-dashed border-slate-200 bg-slate-50 p-6 text-center">
                        ...
                        <p className="mt-2 text-sm font-medium text-slate-600">No bank details submitted</p>
                        <p className="text-xs text-slate-400">The tenant has not submitted banking information yet.</p>
                      </div>
                    )}
                  </section>
                </div>
