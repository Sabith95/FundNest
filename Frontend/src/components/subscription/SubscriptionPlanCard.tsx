...
import { Ban, Check, RotateCcw, X, type LucideIcon } from "lucide-react";
import type { SubscriptionPlan } from "../../types/subsctiption.types";
...
export default function SubscriptionPlanCard({
  ...
  const features = [
    ...
    { label: `Max users: ${formatLimit(plan.maxUsers)}`, included: true },
    { label: "Autopay", included: plan.hasAutopay },
    { label: "Fund suggestions", included: plan.hasFundSuggestions },
  ];
  ...
  return (
    <article
      className={`relative flex h-full flex-col rounded-2xl border transition-shadow ${
        isPopular
          ? "border-transparent bg-gradient-to-b from-indigo-950 to-slate-900 text-white shadow-xl shadow-indigo-950/20 lg:-translate-y-3"
          : "border-slate-200 bg-white text-slate-900 hover:shadow-md"
      } ${!plan.isActive && variant === "admin" ? "opacity-70" : ""}`}
    >
      {isPopular && (
        <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-emerald-400 px-3 py-1 text-[11px] font-semibold text-emerald-950">
          Most popular
        </span>
      )}
      ...
      {variant === "admin" && (
        ...
        >
          <button
            type="button"
            onClick={() => onEdit?.(plan)}
            className={`flex flex-1 items-center justify-center rounded-lg px-4 py-2.5 text-sm font-semibold ${
              isPopular
                ? "bg-white text-slate-900 hover:bg-white/90"
                : "bg-slate-200 text-slate-700 hover:bg-slate-300"
            }`}
