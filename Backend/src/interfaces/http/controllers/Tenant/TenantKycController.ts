...
export class TenantKycController {
    ...
    uploadKycDocuments = async (
        ...
        try {
            ...
            const { businessRegistrationCertificateKey, ownerIdProofKey } = req.body;
            if (!businessRegistrationCertificateKey || !ownerIdProofKey) {
                throw new BadRequestError(
                   MESSAGES.FILE_UPLOAD.BUSINESS_REGISTRATION_CERTIFICATE_KEY_AND_OWNER_ID_PROOF_KEY_ARE_REQUIRED
                );
            }

            const result =
                await this._uploadKycDocumentsUseCase.execute(
                    tenantId,
                    {
                        businessRegistrationCertificateKey:
                            req.body.businessRegistrationCertificateKey,

                        ownerIdProofKey:
                            req.body.ownerIdProofKey,
                    }
                );
