...
export class TenantRepository extends MongoBaseRepository<Tenant> implements ITenantRepository {
    ...
    async verifyKycDocuments(
        ...
        const doc = await TenantModel.findByIdAndUpdate(
            ...
            {
                $set: {
                    "kycDocuments.businessRegistrationCertificate.verification": verificationData.businessRegistrationCertificateVerification,
                    "kycDocuments.ownerIdProof.verification": verificationData.ownerIdProofVerification
                }
            },
            { new: true, runValidators: true }
        ).lean();
    ...
    async updateOverallStatus(
        ...
        if (onboardingStep !== undefined) updatePayload.onboardingStep = onboardingStep;
        const doc = await TenantModel.findByIdAndUpdate(
            tenantId,
            { $set: updatePayload },
            { new: true, runValidators: true }
