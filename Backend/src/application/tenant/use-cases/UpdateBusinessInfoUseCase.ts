...
export class UpdateBusinessInfoUseCase implements IUpdateBusinessInfoUseCase {
    ...
    async execute(tenantId: string, input: UpdateBusinessInfoDto): Promise<UpdateBusinessInfoResponseDto> {
        ...
        const isValidStep = tenant.onboardingStep === OnboardingStep.REGISTERED || 
                            tenant.onboardingStep === OnboardingStep.OTP_VERIFIED;

        if (!isValidStep && !isReupload) {
            throw new ForbiddenError("Business info update is not allowed at this step.");
        }

        const nextStep = isReupload ? tenant.onboardingStep : OnboardingStep.BUSINESS_INFO_COMPLETED;
        ...
        return { tenant: TenantResponseMapper.toUpdateBusinessInfoResponse(finalTenant) };
    }
}